...
export const getPageTableOfContents = (page, recordMap) => {
  ...
  const toc = getBlockHeader(contents, recordMap, [], pageId, new Set())
 ...
 * 重写获取目录方法
 */
function getBlockHeader(contents, recordMap, toc, pageId, visitedBlockIds) {
  if (!toc) {
    toc = []
  }
  if (!contents) {
  ...
  for (const blockId of contents) {
    ...
    if (block.content?.length > 0) {
      getBlockHeader(block.content, recordMap, toc, pageId, visitedBlockIds)
    } else {
      if (isHeading) {
