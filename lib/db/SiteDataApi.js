...
function cleanPostForClient(post) {
  ...
  expandTransclusionReferences(cleanedPost.blockMap)
...
function cleanNoticeForClient(notice) {
  ...
  expandTransclusionReferences(cleanedNotice.blockMap)
...
function expandTransclusionReferences(blockMap) {
  const blocks = blockMap?.block
  if (!blocks) return

  const getBlock = id => blocks[id]?.value || blocks[id]
  const isAncestor = (ancestorId, blockId) => {
    let current = getBlock(blockId)
    const seen = new Set()
    while (current?.parent_id && !seen.has(current.parent_id)) {
      if (current.parent_id === ancestorId) return true
      seen.add(current.parent_id)
      current = getBlock(current.parent_id)
    }
    return false
  }
  ...
  Object.entries(blocks).forEach(([id, entry]) => {
    ...
    if (block?.type !== 'transclusion_reference') return

    const pointerId = block.format?.transclusion_reference_pointer?.id
    const pointerBlock = getBlock(pointerId)
    const canExpand =
      pointerId &&
      pointerBlock?.type === 'transclusion_container' &&
      !isAncestor(pointerId, id)

    replacements.set(id, canExpand && Array.isArray(pointerBlock.content)
      ? pointerBlock.content
      : [])
  })
  ...
  Object.values(blocks).forEach(entry => {
    ...
    block.content = block.content.flatMap(id => replacements.get(id) || [id])
  })
